...
describe('Campaigns stats API tests', async function () {
  
    it('should return status code 200 with valid key', async function () {
        const response = await valid_key('/api/campaigns/stats');
    ...
    it('should return status code 403 with invalid key', async function () {
        const response = await invalid_key('/api/campaigns/stats');
        body = JSON.parse(JSON.stringify(response.body));
